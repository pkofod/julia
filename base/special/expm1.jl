...
expm1_invln2(::Type{Float64}) = 1.44269504088896338700e+00
expm1_huge(T::Type{Float64}) = 56.0*expm1_ln2(T)
expm1_overflow(::Type{Float64}) = 7.09782712893383973096e+02
expm1_underflow(::Type{Float64}) = 2.0^-54
...
expm1_underflow(::Type{Float32}) = 2f0^-25

expm1_ln2(T) = expm1_ln2_hi(T)+expm1_ln2_lo(T)

expm1_set_highword(x, hw) = reinterpret(Float64, hw|(reinterpret(UInt64, x)<<32)>>32)
...
expm1_p(hfxs::Float32) = @horner(hfxs, -3.3333212137f-2, # Q1
                                        1.5807170421f-3) # Q2

expm1_two_pow_k(::Type{Float32}, k) = reinterpret(Float32, 0x3f800000+(k<<23))
expm1_two_pow_k(::Type{Float64}, k) = reinterpret(Float64, UInt64(0x3ff00000+(k<<20))<<32)

expm1_one_m_2_pow_k(::Type{Float32}, k) = reinterpret(Float32, (0x7f-k)<<23) 
expm1_one_m_2_pow_k(::Type{Float64}, k) = reinterpret(Float64, (UInt64(0x3ff00000 - (0x200000>>k))<<32)|reinterpret(UInt64, 1.0))
...
function expm1(x::T) where T<:Union{Float32, Float64}
	...
	if absx >= expm1_huge(T) # quit early for large absolute values of x
	    if absx >= expm1_overflow(T) # largest value T can take before overflowing
		    return ifelse(x == T(Inf), x, -T(1.0))
        end
    ...
    if k==0
        ...
        #two_pow_k = expm1_two_pow_k(T, k) # 2^k
        e  = (x*(e-c)-c)
	    e -= hfxs
        if k == -1
	    ...
	    if (k <= -2 || k > 56) # suffice to return exp(x)-1
            ...
            ebk = expm1_big_k(T)
            if k == ebk
                y = y*T(2.0)*T(2.0)^(ebk-1) # 127 for Float32
            else
                y = y*T(2.0)^k
            end
	        return y-T(1.0)
	    ...
	    if k < expm1_small_k(T)
	        t = T(1.0)-T(2.0)^-k # expm1_one_m_2_pow_k(T, k)# t=1-2^-k
	       	y = t-(e-x)
		    y = y*T(2.0)^k
	    else
#            t = expm1_two_pow_k(T, k) # 2^-k
            t = T(2.0)^-k
	       	y = x-(e+t)
	       	y += T(1.0)
		    y = y*T(2.0)^k
